...
describe("accounts submitted tests", () => {
    ...
    it("should throw error when session not properly set", async () => {
        resetMockSession();
        expect(await getRequestWithCookie(PrefixedUrls.CONFIRMATION, server)).rejects.toThrow; // eslint-disable-line @typescript-eslint/no-unused-expressions
    });
    ...
    it("Should contain email error when no email provided", async () => {
        ...
        expect(getRequestWithCookie(PrefixedUrls.CONFIRMATION, server)).rejects.toThrow; // eslint-disable-line @typescript-eslint/no-unused-expressions
    });
