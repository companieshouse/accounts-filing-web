...
describe("extractCompanyNumberMiddleware", () => {
    ...
    it("should validate and set companyNumber in session when companyNumber is present and valid", () => {
        ...
        expect(req.session?.setExtraData).toHaveBeenCalledWith("companyNumber", "12345678");
    ...
    it("should propagate error when companyNumber validation fails", () => {
        req.params = { companyNumber: "invalid-number" };

        const error = new Error("Invalid company number format");
        (checkCompanyNumberFormatIsValidate as jest.Mock).mockImplementation(() => {
            throw error;
        });
        ...
        expect(checkCompanyNumberFormatIsValidate).toHaveBeenCalledWith("invalid-number");
        expect(req.session?.setExtraData).not.toHaveBeenCalled();
